...
def fast_connect(*args, **kwargs):
    ...
    try:
        ...
        # if 4GB is too much, reduce to -2000000 (2GB)
...
def create_equities_df(df, tickers, sessions, sharadar_metadata_df, show_progress):
    """
    Optimized & Fixed: Vectorized metadata creation with correct index handling.
    """
    ...
    equities_df['start_date'] = subset['firstpricedate'].values
    equities_df['end_date'] = subset['lastpricedate'].values
    equities_df['first_traded'] = subset['firstpricedate'].values
    equities_df['auto_close_date'] = (subset['lastpricedate'] + pd.Timedelta(days=1)).values
    
    equities_df['exchange'] = subset['exchange'].fillna('OTC').replace('None', 'OTC').values
