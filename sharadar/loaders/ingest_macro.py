...
def create_macro_prices_df(start_str: str, calendar=get_calendar('XNYS')):
    ...
    tres_df = tres_df.reindex(sessions).ffill().dropna()#.fillna(method='ffill').dropna()
    ...
    # Frequency: daily
    corp_bond_df = _to_prices_df(pdr.DataReader(['BAMLC0A0CMEY'], 'fred', start, end).ffill(),10400)#.fillna(method="ffill"), 10400)
