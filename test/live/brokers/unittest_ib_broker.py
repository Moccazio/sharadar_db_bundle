...
class TestIBBroker(WithSimParams,
    ...
    def _create_exec_detail(order_id, shares, cum_qty, price, avg_price,
        ...
        exec_detail.orderId = order_id
        exec_detail.shares = shares
        exec_detail.cumQty = cum_qty
        exec_detail.price = price
        exec_detail.avgPrice = avg_price
        exec_detail.time = exec_time
        exec_detail.execId = exec_id
        return exec_detail

    @patch('sharadar.live.brokers.ib_broker.TWSConnection')
    ...
    def test_order_ref_serdes(self):
        ...
        assert deserialized['action'] == order.action
        assert deserialized['qty'] == order.totalQuantity
